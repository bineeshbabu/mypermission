...
import com.my.mypermissions.R;
import com.my.mypermissions.ui.MyPermissionActivity;
...
public abstract class PermissionBuilder<T extends PermissionBuilder> {
    ...
    public PermissionBuilder(Context context) {
        ...
        deniedCloseButtonText = context.getString(R.string.mypermission_close);
        rationaleConfirmText = context.getString(R.string.mypermission_confirm);
        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED;
    }

    protected void checkPermissions() {
        if (listener == null) {
            throw new IllegalArgumentException("You must setPermissionListener() on MyPermission");
        } else if (ObjectUtils.isEmpty(permissions)) {
            throw new IllegalArgumentException("You must setPermissions() on MyPermission");
        }
        ...
        intent.putExtra(MyPermissionActivity.EXTRA_PERMISSIONS, permissions);

        intent.putExtra(MyPermissionActivity.EXTRA_RATIONALE_TITLE, rationaleTitle);
        intent.putExtra(MyPermissionActivity.EXTRA_RATIONALE_MESSAGE, rationaleMessage);
        intent.putExtra(MyPermissionActivity.EXTRA_DENY_TITLE, denyTitle);
        intent.putExtra(MyPermissionActivity.EXTRA_DENY_MESSAGE, denyMessage);
        intent.putExtra(MyPermissionActivity.EXTRA_PACKAGE_NAME, context.getPackageName());
        intent.putExtra(MyPermissionActivity.EXTRA_SETTING_BUTTON, hasSettingBtn);
        intent.putExtra(MyPermissionActivity.EXTRA_DENIED_DIALOG_CLOSE_TEXT, deniedCloseButtonText);
        intent.putExtra(MyPermissionActivity.EXTRA_RATIONALE_CONFIRM_TEXT, rationaleConfirmText);
        intent.putExtra(MyPermissionActivity.EXTRA_SETTING_BUTTON_TEXT, settingButtonText);
        intent.putExtra(MyPermissionActivity.EXTRA_SCREEN_ORIENTATION, requestedOrientation);

        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.addFlags(Intent.FLAG_ACTIVITY_NO_USER_ACTION);
        MyPermissionActivity.startActivity(context, intent, listener);
        MyPermissionBase.setFirstRequest(context,permissions);
    }
